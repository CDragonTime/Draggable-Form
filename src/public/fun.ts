...
export const addQueryParam = function (url, paramObj) {
  const urlQuery =
    url.indexOf('?') > -1 ? qs.parse(url.split('?')[1], { arrayFormat: 'index' }) : {};
  const objQuery =
    typeof paramObj === 'string' ? qs.parse(paramObj, { arrayFormat: 'index' }) : paramObj;
 ...
 * @return {Array}
 */
export const convertListToTreeData = (
  listData,
  idField = 'id',
  parentIdFiled = 'parentId',
  defaultParentId: any = '_auto',
 ...
 * @return {Array}
 */
export const convertListToTreeNoneData = (
  listData,
  idField = 'id',
  parentIdFiled = 'parentId',
  defaultParentId: any = '_auto',
    ...
    if (!limitValue) {
      ...
      if (!disabledFuture) {
        ...
        if (typeof disabledFuture === 'boolean') {
          ...
          // 如果有未来限制
          return (
            value.valueOf() < limitValue.valueOf() ||
            value.valueOf() > dayjs().add(disabledFuture, 'days').valueOf()
          );
        }
      }
    }
