 ...
 */
// tslint:disable
import momentTimeZone from 'dayjs'
...
export const formatTimeWithTimeZone = (value: Date | string | number, format?: string, timezone?: string) => {
...
export const formatDate = (dt, fmt = 'YYYY-MM-DD', timezone = 'Asia/Shanghai') => {
  if (!dt) return ''
  return momentTimeZone.tz(dt, timezone).format(fmt)
}
export const formatHMS = (dt, fmt = 'HH:mm:ss z', timezone = 'Asia/Shanghai') => {
  if (!dt) return ''
  return momentTimeZone.tz(dt, timezone).format(fmt)
}
  ...
  if (isStartDate) {
    ...
    if (!limitValue) {
      ...
      if (typeof disabledFuture === 'boolean') {
        ...
        // 如果有未来限制
        return value.valueOf() > moment().subtract(disabledFuture, 'days').valueOf()
      }
    } else {
      // 如果有结束日期的情况
    ...
    if (!limitValue) {
      ...
      if (typeof disabledFuture === 'boolean') {
        ...
        // 如果有未来限制
        return value.valueOf() > moment().add(disabledFuture, 'days').valueOf()
      }
    } else {
      if (!disabledFuture) {
        return value.valueOf() < limitValue.valueOf()
        ...
        if (typeof disabledFuture === 'boolean') {
          ...
          // 如果有未来限制
          return value.valueOf() < limitValue.valueOf() || value.valueOf() > moment().add(disabledFuture, 'days').valueOf()
        }
      }
    }
  }
...
export function sortFun(prevVal, nextVal, type: 'text' | 'number' | 'date' | 'array' = 'text') {
  switch (type) {
    ...
    case 'date':
      return moment(prevVal).valueOf() - moment(nextVal).valueOf()
    case 'array':
...
// 剔除对象中空值的属性
export function objectTrim(obj, ignore?: string[], delNull = false) {
  return pickby(obj, (v, k) => {
    if (ignore && k.indexOf(ignore) > -1) return true
    if (delNull) return v !== '' && v !== null
    else return v !== '' && v !== null
  })
}
...
// 本地读取文件生成md5
export const createFileMd5 = (file) => {
  return new Promise((resolve, reject) => {
    ...
    fileReader.onload = (e) => {
      ...
      const isSuccess = file.size === result['byteLength']
      // 读取成功，则生成MD5，扔出去。失败就报错
      isSuccess ? resolve(SparkMD5.ArrayBuffer.hash(result)) : reject(new Error('读取出错了'))
    }
    //   读取过程中出错也直接报错
